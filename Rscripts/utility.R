...
analyse_splicing_variants<-function(files_dir=NULL, file_pattern="_spliceaiSNP.vcf", outdir="results", outfile_prefix="splice",annotation_data=annotationData, splice_th=0.7, ptrans_th=95, gq_th=10, dp_th=30, qual_th=10 ){
  ...
  merged_spliceAI_results = merged_spliceAI_results[,count_passing_qual:=uniqueN(sample_variant_ID[gt_GQ>=gq_th & gt_DP>=dp_th & QUAL>=qual_th & FILTER=="PASS"]),by=c("Indiv")]
  merged_spliceAI_results = merged_spliceAI_results[,count_passing_qual_disease_gene:=uniqueN(sample_variant_ID[gt_GQ>=gq_th & gt_DP>=dp_th & QUAL>=qual_th & FILTER=="PASS" & is_disease_causing=="TRUE"]),by=c("Indiv")]
  merged_spliceAI_results = merged_spliceAI_results[,count_passing_qual_disease_gene_potential:=uniqueN(sample_variant_ID[gt_GQ>=gq_th & gt_DP>=dp_th & QUAL>=qual_th & FILTER=="PASS" & is_disease_causing=="TRUE" & (DiseaseVariantOfInterest=="TRUE" | DiseaseVoI_other=="TRUE")]),by=c("Indiv")]
  merged_spliceAI_results = merged_spliceAI_results[,count_passing_qual_disease_gene_potential_filtered:=uniqueN(sample_variant_ID[gt_GQ>=gq_th & gt_DP>=dp_th & QUAL>=qual_th & FILTER=="PASS" & is_disease_causing=="TRUE" & (DiseaseVariantOfInterest=="TRUE" | DiseaseVoI_other=="TRUE") & nsamples<=2]),by=c("Indiv")]
